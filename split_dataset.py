...
def save_file(dataset, filename):
    ...
    with open(filename, 'w', newline='') as file:
        ...
        for domain, intent, level, vp, u in dataset:
            corpus_out.writerow([domain, intent, level, vp] +  u)
...
if __name__ == '__main__':
    ...
    if args.filename:
        ...
        with open(args.filename, newline='') as dataset:
            ...
            #for domain, intent, level, pattern, src, tgt, score in reader:
            for domain, intent, level, pattern, src, src_flat, src_bio, tgt, tgt_flat, tgt_bio in reader:
                uniq_pattern_idx = "#".join([domain, intent, level, pattern])
                if uniq_pattern_idx in corpus:
                    corpus[uniq_pattern_idx].append([src, src_flat, src_bio, tgt, tgt_flat, tgt_bio])
                else:
                    corpus[uniq_pattern_idx] = [[src, src_flat, src_bio, tgt, tgt_flat, tgt_bio]]
